...
const productName = (p: Produto, fallback: string) => p.descricao || p.nome || p.produto || fallback;
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
              ...
              {t.topProducts.heading}
            </h2>
            <p className="text-sm text-muted-foreground">{t.topProducts.hint}</p>
          </div>
          <TabsList className="bg-white/80 shadow-sm">
            <TabsTrigger value="mais-vendidos" className="gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#4e6ae9] data-[state=active]:to-[#6b8cff] data-[state=active]:text-white">
              <Store className="h-4 w-4" /> {interpolate(t.topProducts.topSellers, { count: topVendas.length })}
            </TabsTrigger>
            <TabsTrigger value="mais-comprados" className="gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#764ba2] data-[state=active]:to-[#9b6fd0] data-[state=active]:text-white">
              <ShoppingCart className="h-4 w-4" /> {interpolate(t.topProducts.topPurchases, { count: topCompras.length })}
            </TabsTrigger>
          </TabsList>
        </div>
...
const ProductRankingChart = ({ products, tipo, onProductClick }: { products: Produto[]; tipo: Tipo; onProductClick: (p: Produto, t: Tipo) => void }) => {
  const t = useI18n();
  const fallback = t.topProducts.fallbackName;
  const chartData = products.map((p) => ({
    name: productName(p, fallback).substring(0, 18),
    fullName: productName(p, fallback),
    ...
    <Card className="shadow-lg border-none bg-white/80 backdrop-blur-sm overflow-hidden">
      ...
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          {interpolate(t.topProducts.rankingByValue, { side: tipo === "venda" ? t.topProducts.sales : t.topProducts.purchases })}
          <Badge variant="outline" className="ml-auto text-xs">{t.topProducts.currentVsReform}</Badge>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={Math.max(280, products.length * 40)}>
          <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 30 }}>
            ...
            />
            <Bar dataKey="Valor Atual" name={t.topProducts.currentValue} fill={`url(#${gradientId}Atual)`} radius={[0, 6, 6, 0]} barSize={16}
              cursor="pointer" onClick={(_, index) => onProductClick(products[index], tipo)} />
            <Bar dataKey="Valor Reforma" name={t.topProducts.reformValue} fill={`url(#${gradientId}Reforma)`} radius={[0, 6, 6, 0]} barSize={16}
              cursor="pointer" onClick={(_, index) => onProductClick(products[index], tipo)} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
...
const ProductCard = ({ product, rank, onClick }: { product: Produto; rank: number; onClick: () => void }) => {
  ...
  const name = productName(product, t.topProducts.fallbackName);
        ...
        <CardContent className="pt-10 pb-4 px-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{t.topProducts.current}</span>
              <span className="text-sm font-bold" style={{ color: COLORS.blue }}>{formatCurrencyShort(product.valor_total ?? 0)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{t.topProducts.reform}</span>
...
const ProductDetailDialog = ({ product, tipo, open, onClose }: { product: Produto | null; tipo: Tipo; open: boolean; onClose: () => void }) => {
  ...
  // Bar comparison data
  const barData = [
    { name: tipo === "compra" ? t.topProducts.totalCost : t.topProducts.salePrice, Atual: product.valor_total ?? 0, Reforma: product.total_reforma ?? 0 },
  ];
  if (product.creditos != null || product.debitos != null) {
    barData.push({ name: tipo === "compra" ? t.topProducts.credits : t.topProducts.debits, Atual: product.creditos ?? product.debitos ?? 0, Reforma: product.creditos_reforma ?? product.debitos_reforma ?? product.ibs_cbs ?? 0 });
  }
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          ...
          <DialogTitle className="flex items-center gap-3 text-lg">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground font-normal mt-0.5">
                {tipo === "compra" ? t.topProducts.purchasedProduct : t.topProducts.soldProduct}
                {product.quantidade != null && ` • ${interpolate(t.topProducts.quantity, { n: formatNumber(product.quantidade) })}`}
              </div>
            </div>
          </DialogTitle>
          <DialogDescription className="sr-only">{t.topProducts.dialogDescription}</DialogDescription>
        </DialogHeader>
        ...
        <div className={`rounded-xl p-4 flex items-center justify-between ${
          ...
          <div>
            <p className="text-sm font-medium text-muted-foreground">{interpolate(t.topProducts.reformImpactOn, { target: tipo === "compra" ? t.topProducts.cost : t.topProducts.price })}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          ...
          <div className="bg-muted/30 rounded-xl p-4">
            <h4 className="text-sm font-semibold mb-3">{t.topProducts.valueComparison}</h4>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={barData}>
                ...
                <Bar dataKey="Atual" name={t.topProducts.current} fill="url(#gradAtualDialog)" radius={[6, 6, 0, 0]} />
                <Bar dataKey="Reforma" name={t.topProducts.reform} fill="url(#gradReformaDialog)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          {radarData.length >= 3 && (
            <div className="bg-muted/30 rounded-xl p-4">
              <h4 className="text-sm font-semibold mb-3">{t.topProducts.taxBreakdown}</h4>
              <ResponsiveContainer width="100%" height={180}>
                <RadarChart data={radarData}>
                  <PolarGrid stroke="#e2e8f0" />
                  <PolarAngleAxis dataKey="metric" tick={{ fontSize: 10, fontWeight: 600 }} />
                  <PolarRadiusAxis tick={{ fontSize: 8 }} />
                  <Radar name={t.topProducts.current} dataKey="Atual" stroke={COLORS.blue} fill={COLORS.blue} fillOpacity={0.2} strokeWidth={2} />
                  <Radar name={t.topProducts.reform} dataKey="Reforma" stroke={COLORS.purple} fill={COLORS.purple} fillOpacity={0.2} strokeWidth={2} />
        ...
        {/* Detail Grid */}
        <div className="mt-2">
          <h4 className="text-sm font-semibold mb-3">{t.topProducts.allIndicators}</h4>
