...
| `src/lib/export.ts` | `toCsv(report)` serialises purchases and sales as a pt-BR CSV (`;` / `,` / UTF-8 BOM). `downloadCsv` triggers the browser download. `printReport` opens `window.print()` for the one-page PDF path. | `export.test.ts` |
...
| `src/pages/Index.tsx` | State (filters, data, loading, error, demo), fetching, auto refresh, page layout, CSV and PDF export buttons. | `Index.test.tsx` |
...
| `src/components/dashboard/TopProducts.tsx` | Ranking chart, product cards, the detail dialog and print-only top-10 tables. | `TopProducts.test.tsx` |
