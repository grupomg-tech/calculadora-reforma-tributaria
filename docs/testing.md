...
| Suite | Scope |
| --- | --- |
| `src/lib/report.test.ts` | `parseApiResponse` (plain, `dados` wrapper, HTML, invalid JSON), `computeImpactDelta`, `deriveBurdenBar`, `derivePieData`, `deriveComparativo`, `rankTopProducts` |
| `src/lib/export.test.ts` | `toCsv` (empty report, fixture rows, `;` quoting, UTF-8 BOM, pt-BR decimals), the download helper, `printReport` / print title and meta line |
...
| `src/pages/Index.test.tsx` | Fetch on mount with the right query string, `dados` wrapper, HTTP error + fallback button, network error message, `?demo=1` without network, recalculation on rate change, CSV and PDF export (disabled until loaded; download / `window.print()` in demo mode), filters from the query string |
| `src/components/dashboard/FilterPanel.test.tsx` | Rendering, setters, submit, disabled while loading, auto-refresh toggle |
| `src/components/dashboard/TopProducts.test.tsx` | Empty state, top-10 ordering, impact badges, detail dialog, purchases tab, print-only ranking tables |
